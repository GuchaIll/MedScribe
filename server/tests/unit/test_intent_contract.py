"""
Locks the copilot runtime contract (agent refactor Phase 0, contract 1.1):
two-stage intent gate, intervention policy, executor-owned plans, shared tool
registry, Lane B outputs, and the golden sets.
...
class TestGateSchema:
    def test_contract_version(self):
        assert RUNTIME_CONTRACT_VERSION == "1.1"
    ...
    def test_valid_decisions_pass(self):
        ...
        for decision in (explicit, ambient_silent, research_chip):
            assert validate_intent_decision(decision) == []

    @pytest.mark.parametrize(
        "build, fragment",
        [
            (lambda: {**_decision(), "plan": ["retrieve_labs"]}, "gate must not emit"),
...
class TestExecutorContract:
    def test_every_task_has_an_executor_mode(self):
        assert EXECUTOR_MODES == ("fixed", "bounded_agent")
        assert set(TASK_EXECUTOR_MODE) == set(TASK_SUBTYPES)
        assert set(TASK_EXECUTOR_MODE.values()) <= set(EXECUTOR_MODES)

    def test_only_trajectory_and_compare_are_agents(self):
        agents = {task for task, mode in TASK_EXECUTOR_MODE.items() if mode == "bounded_agent"}
        assert agents == {"trajectory", "compare"}

    def test_fixed_chains_cover_every_fixed_task_with_assist_tools(self):
        fixed = {task for task, mode in TASK_EXECUTOR_MODE.items() if mode == "fixed"}
        assert set(FIXED_TOOL_CHAINS) == fixed
        for task, chain in FIXED_TOOL_CHAINS.items():
            for tool_id in chain:
                assert TOOL_SPECS[tool_id]["assist_callable"], (task, tool_id)

    def test_factoid_chain_tries_structured_before_documents(self):
        chain = FIXED_TOOL_CHAINS["factoid"]
        assert chain.index("retrieve_labs") < chain.index("retrieve_docs")
        assert chain[-1] == "retrieve_docs"

    def test_pull_safety_uses_the_push_safety_tool(self):
        assert "safety_check" in FIXED_TOOL_CHAINS["safety"]
        assert TOOL_SPECS["safety_check"]["lane_b_stages"] == ("safety_and_validate",)

    def test_valid_agent_step_runs(self):
        step = {"tool_id": "retrieve_labs", "args": {"analyte": "rbc", "time_range": "last_2_years"}}
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({"tool_id": "safety_check", "args": {}}, 0, 0, "allowlist"),
            ({"tool_id": "retrieve_for_candidates", "args": {}}, 0, 0, "allowlist"),
            ({"tool_id": "retrieve_labs", "args": {"patient_id": "p1"}}, 0, 0, "scope keys"),
            ({"tool_id": "retrieve_docs", "args": {"filters": {"tenant_id": "t1"}}}, 0, 0, "filters.tenant_id"),
            ({"tool_id": "retrieve_everything", "args": {}}, 0, 0, "unknown tool"),
            ({"tool_id": "retrieve_labs", "args": []}, 0, 0, "args must be an object"),
            ({"tool_id": "retrieve_labs", "args": {}}, MAX_AGENT_TOOL_CALLS, 0, "call cap"),
            ({"tool_id": "retrieve_labs", "args": {}}, 0, AGENT_WALL_CLOCK_BUDGET_MS, "wall-clock"),
        ],
    )
    def test_invalid_agent_step_is_refused(self, step, calls_made, elapsed_ms, fragment):
...
class TestSharedToolRegistry:
    def test_tool_ids_stages_and_callers_are_frozen(self):
        ...
        assert TOOL_CALLERS == ("lane_b_stage", "fixed_executor", "bounded_agent")
    ...
    def test_retrieval_is_shared_by_lane_b_fixed_chains_and_agents(self):
        for tool_id in ("retrieve_session", "retrieve_labs", "retrieve_docs", "retrieve_visits"):
            spec = TOOL_SPECS[tool_id]
            assert spec["assist_callable"], tool_id
            assert spec["agent_allowlisted"], tool_id
            assert "enrich" in spec["lane_b_stages"], tool_id
            assert spec["requires_citations"], tool_id
        lane_b_only = TOOL_SPECS["retrieve_for_candidates"]
        assert not lane_b_only["assist_callable"]
        assert lane_b_only["lane_b_stages"] == ("enrich",)

    def test_agent_allowlist_is_read_and_scratch_only(self):
        assert AGENT_ALLOWLIST == {
            "retrieve_session",
            "retrieve_labs",
            "retrieve_docs",
            "retrieve_visits",
        ...
        assert all(TOOL_SPECS[t]["writes"] in ("none", "session_scratch") for t in AGENT_ALLOWLIST)

    def test_note_generation_is_neither_a_compile_stage_nor_agent_tool(self):
        spec = TOOL_SPECS["generate_note"]
        assert spec["lane_b_stages"] == ()
        assert spec["assist_callable"]
        assert not spec["agent_allowlisted"]

    def test_scope_is_injected_never_model_supplied(self):
        assert {"patient_id", "tenant_id"} <= SCOPE_ARG_KEYS
        errors = validate_model_tool_args("retrieve_labs", {"items": [{"patient_id": "p1"}]})
        assert any("items[0].patient_id" in e for e in errors), errors
        assert validate_model_tool_args("retrieve_labs", {"analyte": "rbc"}) == []

    def test_grounded_result_passes(self):
        assert validate_tool_result(_tool_result()) == []

    def test_no_source_miss_is_valid(self):
        miss = _tool_result(ok=False, data={}, citations=[], error=NO_SOURCE_ERROR)
    ...
    def test_safety_check_may_return_without_citations(self):
        result = _tool_result(tool_id="safety_check", citations=[], data={"alerts": []})
...
class TestLaneBContracts:
    ...
    def test_self_correction_leaves_one_active_fact(self):
        # "penicillin... actually no, it was amoxicillin" (plan §4.7)
...
class TestTaskGoldenSet:
    def test_well_formed(self):
        rows = _load("intent_task_v1.jsonl")
    ...
    @pytest.mark.parametrize("row", _load("intent_task_v1.jsonl"), ids=lambda r: r["id"])
    ...
    def test_expected_tools_only_on_agent_rows_and_within_allowlist(self):
        rows = _load("intent_task_v1.jsonl")
        for r in rows:
            is_agent = TASK_EXECUTOR_MODE[r["expected_task_subtype"]] == "bounded_agent"
            assert ("expected_tools" in r) == is_agent, r["id"]
            assert set(r.get("expected_tools", [])) <= AGENT_ALLOWLIST, r["id"]

        compare_rows = [r for r in rows if r["expected_task_subtype"] == "compare"]
        assert len(compare_rows) >= 3
        assert all("compare_findings" in r["expected_tools"] for r in compare_rows)
...
class TestContractDocSync:
    ...
    def test_doc_provisional_numbers_match_code(self):
        text = _CONTRACT_DOC.read_text()
        for number in (AMBIENT_CHIP_MIN_CONFIDENCE, MAX_AGENT_TOOL_CALLS, AGENT_WALL_CLOCK_BUDGET_MS):
            assert f"`{number}`" in text, number

    def test_doc_has_no_superseded_v1_0_names(self):
        text = _CONTRACT_DOC.read_text()
        for stale in ("addressed_voice", "intent_golden_v1", "DEFAULT_RETRIEVAL_PLAN", "copilot_factoid"):
