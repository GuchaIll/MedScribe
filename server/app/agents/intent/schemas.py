"""
Intent gate contract — v1.2 (agent refactor Phase 1A, #50).
...
- Stage B — task subtype + slots on copilot-directed input. Errors here are
  recoverable (the clinician re-asks). In 1.2, Stage B also emits sub_asks
  with calibrated per-class scores; the executor's dispatch_assist() picks the
  next step from those (§12.3–12.4).
...
Changes from 1.1:
- EvidenceClass literal added (§12.3).
- SubAsk TypedDict added (§12.3).
- TaskSlots gains sub_asks and readings_conflict (§12.3).
"""
...
# Evidence classes used by Stage B sub-asks (§12.3).
# Mirrors the TaskSubtype job values plus retrieval classes and no_workflow.
EvidenceClass = Literal[
    ...
    "doc_ingest",
    "session_control",
    # Sentinel: no known workflow covers this class; dispatch -> planning_agent
    "no_workflow",
]
...
EVIDENCE_CLASSES: Tuple[str, ...] = get_args(EvidenceClass)
...
# ---------------------------------------------------------------------------
# Stage B: sub-asks (§12.3)
# ---------------------------------------------------------------------------

class SubAsk(TypedDict):
    """A single evidence need declared by Stage B.

    sub_ask_id is unique within the request. class_scores maps each evidence
    class to a calibrated confidence; a class is accepted at
    SUB_ASK_MIN_CONFIDENCE (config.py). depends_on lists sub_ask_ids whose
    results must be available before this sub-ask can be answered.

    Stage B never emits tool ids or tool order — sub_asks describe evidence
    only (§12.8 invariant 1).
    """
    sub_ask_id: str
    span: str                        # the portion of the clinician's ask this covers
    class_scores: Dict[str, float]   # evidence_class -> calibrated confidence 0..1
    entity_hints: List[str]          # e.g. ["rbc", "hemoglobin"]
    domain_hints: List[str]          # clinical sectors or scope library ids
    time_range: Optional[str]        # e.g. "last_2_years", "since_last_visit"
    depends_on: List[str]            # sub_ask_ids this one depends on
...
class TaskSlots(TypedDict):
    ...
    sub_asks: List[SubAsk]          # Stage B evidence breakdown (§12.3); [] until Stage B lands
    readings_conflict: bool         # True when sub-asks reference contradictory readings (§12.3)
