...
**Status:** frozen, pending team sign-off (see §11)  
**Contract version:** `1.1`  
**Date:** 2026-09-10  
**Phase:** 0 of [agent_refactor_plan.md](./agent_refactor_plan.md)  
**Code:** `server/app/agents/intent/` (gate schema, policy, executor contract), `server/app/agents/tool_contracts.py` (shared tool registry), `server/app/agents/compile_contracts.py` (Lane B outputs). Nothing is wired into routes or `graph.py` yet.  
**Tests:** `server/tests/unit/test_intent_contract.py`  
**Golden sets:** `server/tests/fixtures/intent_task_v1.jsonl`, `server/tests/fixtures/intent_addressee_v1.jsonl`
...
4. **Fixed vs bounded agent per task:** trajectory and compare are bounded agents; everything else is fixed (§2).
5. **Tool registry hardening:** runtime-injected patient/tenant scope, agent allowlist, three caller kinds. Registry work moves to Phase 1 (§6).
...
There is no global choice between fixed pipelines and agents. It is chosen per
lane and task, and tools are shared so the choice can flip later without a
rewrite.

**Fixed** when the plan is known up front (roughly ≤ 3 steps), the output is
durable or safety-critical, or the result must be reproducible.  
**Bounded agent** when the next step depends on the last result, and the output
is ephemeral, cited, and read by a human before anything persists.
...
| `trajectory` | `bounded_agent` | depends on what exists for this patient |
| `compare` | `bounded_agent` | retrieve / ingest / compare order is data-dependent |

`TASK_EXECUTOR_MODE` in `executor_contract.py` is the code form of this table.
...
| `trajectory` | longitudinal synthesis | "How has her respiratory system trended over two years?" | `assist` | `bounded_agent` |
| `compare` | new vs old: session vs history, doc vs doc, visit vs visit | "Compare the new findings with the old blood tests." | `assist` | `bounded_agent` |
...
|------|------------|-------|
| `factoid` | `retrieve_session` → `retrieve_labs` → `retrieve_docs` | fallback chain: stop at the first grounded hit |
| `safety` | `retrieve_session` → `safety_check` | same `safety_check` tool as Lane B push |
...
### 5.2 Bounded agent (trajectory, compare)

```text
while calls < max and elapsed < budget and not done:
    model proposes next allowlisted tool + args   # no scope keys
    runtime injects session scope, runs tool, appends ToolResult + receipt
answer draft -> grounding check -> render or refuse
```

- Allowlist: tools with `agent_allowlisted` in §6.2. Read-only or session scratch; no Lane C writes.
...
- Prefer session artifacts before re-ingesting; receipts record `cache_hit`.
- Chart changes implied by a compare go to review. The agent never persists.
...
| `fixed_executor` | a fixed assist chain or job (§5.1) |
| `bounded_agent` | a trajectory / compare agent turn (§5.2) |
...
| Tool id | Category | Lane B stage | Fixed executor | Agent allowlist | Writes | Must cite when ok |
|---------|----------|--------------|----------------|-----------------|--------|-------------------|
| `retrieve_session` | retrieval | `enrich` | yes | yes | none | yes |
| `retrieve_labs` | retrieval | `enrich` | yes | yes | none | yes |
...
| `compare_findings` | compare | `enrich` (optional helper) | yes | yes | none | yes |
| `generate_note` | note | — (demoted from graph) | yes | no | session draft | no |
| `safety_check` | safety | `safety_and_validate` | yes | no | none | no |
...
`Citation`: `source_kind`, `source_id`, `snippet`, `observed_at`, `score`.
...
```text
safety_and_validate -> safety_check -> SafetyAlertCard -> Redis alert channel + UI card
```
...
2. The gate never emits tool ids, tool order, or scope keys.
...
5. An assist answer renders only after the grounding check passes. With no source, it says so.
6. Only `trajectory` and `compare` run as bounded agents, inside the allowlist, call cap, and wall-clock budget.
7. Lane B, fixed executors, and agents call the same tool implementations through one registry.
...
**`intent_task_v1.jsonl`**: explicit-channel rows for Stage B.
...
| `expected_tools` | required on `trajectory` / `compare` rows only: tools a correct agent run must touch |
| `time_range`, `domain_hints`, `entity_hints`, `source_prefs`, `note` | optional slot labels |
...
Provisional numbers (chip threshold `0.6`, agent cap `6`, budget `8000` ms,
push-alert severities) can change without a version bump when backed by eval or
latency evidence.
...
- [ ] Team sign-off: §2 matrix and §3 gate contract
- [ ] Team sign-off: §4 intervene rules
- [ ] Team sign-off: §6 tool registry and §7 Lane B outputs
